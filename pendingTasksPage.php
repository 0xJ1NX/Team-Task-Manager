...
if (isset($_GET['taskId']) && isset($_GET['action'])) {
    ...
    if ($action == 'start') {
        ...
        if ($result) {
            header("location: pendingTasksPage.php");
        }
    ...
    <main>
        ...
        <div class="table-container">
            <table id="pendingTable">
                ...
                $sql = "SELECT * FROM tasks WHERE assigned_to = '$user_id' AND status = 0 ORDER BY priority DESC";
                $res = $conn->query($sql);
                ...
                if ($res->rowCount() > 0) {
                    while ($row = $res->fetch()) {
                        ...
                        <td>
                            <a href="?taskId=<?php echo $row['task_id'] ?>&action=start" class="button-blue">Start</a>
                        </td>
                        <td>
                            <a href="?taskId=<?php echo $row['task_id'] ?>&action=delete" class="button-red" >Delete</a>
                        </td>
